...
/*define global variables for all threads*/
scene_t scene;
int nthreads;
float values[width][height][3];
...
void* perform_work(void* argument){
    ...
    /* for every pixel */
    for( int px=*(int*)argument; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            values[px][py][0] = scaled_color[0];
            values[px][py][1] = scaled_color[1];
            values[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
